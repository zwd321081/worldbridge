...
API keys and settings are stored in Chrome extension sync storage, `chrome.storage.sync`, and are not written to files in this project. If Chrome account sync is enabled, these settings may sync to other Chrome browsers under the same account. This extension is intended for personal use; do not share a browser profile or packaged extension that contains a real API key.
